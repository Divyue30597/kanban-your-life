...
import { board, card, column, InputType } from "@/types/types";
import { deleteColumns, updateCardColumn } from "@/features/board/boardsSlice";
...
export default function Body() {
  ...
  const board: board = selector[pathname.split("/")[1]];
              ...
              pathname.split("/")[1] === col.boardId && (
                <div key={col.id} className={styles.column}>
                  <div className={styles.column_heading}>
                    <h2>{col.name}</h2>
                    <div
                      className={`${styles.column_heading_buttons} flex_row`}
                    >
                      <Modal>
